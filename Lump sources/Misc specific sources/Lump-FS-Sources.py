...
def main():
  ...
  with RMc.create_db_connection(database_Path, [RMNOCASE_Path]) as conn:
    ...
    for source in conn.execute(sql):
      ...
      citation_name = "{}, {}, \"{}\"".format(principal, fields['record_name'], collection)
      
      if collection not in fs_sources:
        fs_source = create_fs_source(conn, collection, fs_template_id, fields['url'])
...
def create_fs_source(conn: Connection, collection, template_id, url) -> dict[str, dict[str,str]]:
    ...
    if new_id:
      ...
      raise RMc.RM_Py_Exception("Unable to create new FamilySearch source: " + collection)
