...
@contextmanager
def create_db_connection(db_file_path, db_extension_file_path_list) -> Generator[Connection, Any, None]:
...
class SourceTemplateField:
  ...
  def to_xml(self):
    ...
    ET.SubElement(field, "DisplayName").text = self.display if self.display else self.name
...
def add_weblink(conn: Connection, name, url, owner_id, owner_type: OwnerType):
  run_sql(conn, INSERT_WEBLINK_STMT, (owner_type,
          owner_id, name, url, RMdate.get_MOD_DATE()))


def add_weblinks(conn: Connection, data=None, **kwargs):
  if not data:
    for (a, b) in itertools.combinations([len(a) for a in kwargs.values() if type(a) is list], 2):
      if a != b:
...
def create_source(conn: Connection, name: str, template_id: str | int, fields: ET.Element, url=None):
  sql = """\
...
VALUES (
    ...
    """
  source_id = run_sql(conn, sql, (name, template_id,
                      ET.tostring(fields), RMdate.get_MOD_DATE()))
  if not source_id:
    raise RM_Py_Exception(f"unable to create new source {name} {template_id}\n{ET.tostring(fields)}")
  if url:
...
def create_citation(conn: Connection, source_id: str, ref_num: str, name: str, fields: ET.Element, url = None):
  """
...
def create_citations(conn: Connection, data=None, **kwargs):
  ...
  next_citation_id = conn.execute(
    'SELECT MAX(CitationID) FROM CitationTable;').fetchone()[0] + 1
...
def create_citation_link(conn: Connection, citation_id, owner_id, owner_type: OwnerType):
  run_sql(conn, INSERT_CITATION_LINK_STMT, (citation_id,
          owner_type, owner_id, RMdate.get_MOD_DATE()))
...
def create_xml_fields(fields: dict, G_DEBUG=False):
  ...
  root.extend([create_xml_field(name, value)
...
INSERT INTO AddressLinkTable (
  ...
  ?,?,?,0,"",?
    ...
    """
  conn.execute(sql_add_repo, (OwnerType.SOURCE, repo_id,
               source_id, RMdate.get_MOD_DATE()))
