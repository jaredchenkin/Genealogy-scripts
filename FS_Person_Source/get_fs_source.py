...
sys.path.append(
  str(Path.resolve(Path(__file__).resolve().parents[1] / 'RMpy package')))
...
def main():
  ...
  fs = Session(config['CREDENTIALS']['FS_USERNAME'], config['CREDENTIALS']['FS_PASSWORD'], verbose=False)
  tree = Tree(fs)
  ...
  with RM.create_db_connection(database_Path, [RMNOCASE_Path]) as conn:
    ...
    print(f"Found {person.name.given} {person.name.surname} in RootsMagic with ID number {rmID}")
    ...
    while True:
      ...
      again = input(f"Do you want to add another source to {person.name.given} [y/N]? ")
...
SELECT et.EventID AS id, et.Date, ft.Name, pt.Name as Place, et.Details
  ...
  while True:
    response = input("Do you want to associate this source with any facts?\n" +
...
def get_name_targets(conn, rid) -> list[str]:
  ...
  for i, n in enumerate(names):
    print(
      f"{i + 1}: {n['given']} {n['surname']} {'(Primary)' if i == 0 else ''}")

  targets = []
  
  while True:
    response = input("Do you want to associate this source with any names?\n" +
...
def get_fs_sources(fs: Session, person: Indi) -> list[Source]:
  print(f"Getting FamilySearch for sources of {person.name.given} {person.name.surname} [{person.fid}]...")
...
title = coll['titles'][0]['value'] if coll['titles'][0]['lang'] = "en_US
  """
def process_source(conn: Connection, rid: int, fs_source: Source, targets: dict[str, list]):
  ...
  if source_id:
    ...
    source_data = fs_source.tree.fs.get_url(f"/platform/sources/descriptions/{fs_source.fid}")
    try:
      ...
      print("No information on the collection available, you must enter it to the source object manually")
      print(e.with_traceback())
    
  match source_info['type']:
    case 'Birth' | 'Death' | 'Marriage':
      if not source_id:
        source_id = create_vitals_source(conn, source_name, source_root, source_info, collection_url)
      new_citation_fields = build_vital_citation_fields(principal_name, citation_fields['record_date'])
    case 'Census':
      if "United States" in source_info['title']:
        source_id = create_us_fed_census_source(conn, source_name, source_root, source_info, collection_url)
      elif 'UK' in source_info['title']:
        source_id = create_uk_census(conn, source_name, source_info, source_root, collection_url)
      else:
        source_id = create_us_state_census(conn, source_name, source_info, source_root, collection_url)
    case _:
      source_id = make_new_source(conn, source_name, source_root, 1)
    
  print(f"Using source {source_name} with source ID {source_id}")

  citation_id = create_citation(conn, fs_source, source_id, citation_name, new_citation_fields, fs_source.url)
  print(f"Created new citation with ID {citation_id}")
  
  create_citation_links(conn, citation_id, targets)
  print(f"Completed migrating source {source_name} for {principal_name} from FamilySearch to RootsMagic!")
...
def create_us_state_census(conn, source_name, source_root, source_info, collection_url):
    ...
    source_id = make_new_source(conn, source_name, source_root, template_id,  new_source_fields, collection_url)
    return source_id
...
def create_us_fed_census_source(conn, source_name, source_root, source_info, collection_url):
    template_id = 43
    new_source_fields = make_us_federal_census_fields(source_info)
    source_id = make_new_source(conn, source_name, new_source_fields, source_root, collection_url, template_id)
    return source_id
...
def get_collection_url(source_data):
    return source_data['sourceDescriptions'][0]['identifiers']['http://gedcomx.org/Collection'][0]
...
def create_citation(conn, fs_source: Source, rm_source_id, name: str, fields: list[dict], url):
  """
  :returns: RM citation ID
  """
  print(f"Creating new citation {fs_source.title} to source {rm_source_id} and url {url} with fields:\n{fields}")
