...
names that match given keywords. It will change the place to match a preconfigured,
standardized place name and link it to events based on the year of the event and the
date range assigned to that place.

## Setup

There are two object types defined in [`place.py`](./place.py): `Place` and `PlaceGroup`.
...
Each string is wrapped in `%`'s, so sqlite will search the whole name field for the string regardless.
Since it still just goes into a `Name LIKE ?` clause, you can add additional `%` or `_` wildcards as needed.
SQLite string queries also default to case insensitive.
