...
cit_re = re.compile(r'FamilySearch \((?P<url>.+?) : (?P<date>[^)]*)\), (?P<record_name>[\w\s\d]+)(?:, (?P<record_date>.+?))?[.;]')
...
SELECT st.Name, st.SourceID, st.TemplateID
  ...
  """
  return { s['Name']: {'id': s['SourceID'], 'template': s['TemplateID']} 
...
SELECT st.SourceID, st.TemplateID
  ...
  if len(sources) > 1:
    raise RM.RM_Py_Exception(f"More than 1 FamilySearch source in RootsMagic with the name {name}. Please Lump first.") 
  if sources:
